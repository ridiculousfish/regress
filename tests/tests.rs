...
fn test_unicode_syntax_lenient_tc(tc: TestConfig) {
    // Test that the 'L' flag allows non-unicode syntax while maintaining unicode semantics

    // Legacy octal escape should work with 'L' flag
    tc.compilef(r"\141", "L").match1f("a").test_eq("a");

    // Control character escape should work with 'L' flag
    tc.compilef(r"\cA", "L").match1f("\x01").test_eq("\x01");

    // Identity escape of unrecognized chars should work with 'L' flag
    tc.compilef(r"\ ", "L").match1f(" ").test_eq(" ");

    // Unbalanced brackets should be allowed with 'L' flag
    tc.compilef(r"a]", "L").match1f("a]").test_eq("a]");

    // Invalid braced quantifiers should be treated as literals with 'L' flag
    tc.compilef(r"a{", "L").match1f("a{").test_eq("a{");

    // Unicode case folding should still work with 'L' flag
    tc.compilef(r"μ", "iL").match1f("Μ").test_eq("Μ");

    // Unicode property escapes should work with 'L' flag
    tc.compilef(r"\p{L}", "L").match1f("a").test_eq("a");

    // Test that 'L' flag doesn't break standard unicode features
    tc.compilef(r"\u{41}", "L").match1f("A").test_eq("A");
}
...
#[test]
fn test_issue26_identity_escapes() {
    test_with_configs(test_issue26_identity_escapes_tc)
}

fn test_issue26_identity_escapes_tc(tc: TestConfig) {
    // Test for GitHub issue #26: https://github.com/ridiculousfish/regress/issues/26
    // Invalid character escape should be accepted in lenient mode but rejected in unicode mode

    // The specific pattern from the issue: /,\\;/
    let pattern = r",\;";

    // Should work in non-unicode mode (default)
    tc.compilef(pattern, "").match1f(",;").test_eq(",;");

    // Should fail in strict unicode mode
    test_parse_fails_flags(pattern, "u");

    // Should work with the new lenient flag
...
#[test]
fn test_unicode_syntax_lenient() {
    test_with_configs(test_unicode_syntax_lenient_tc)
}
