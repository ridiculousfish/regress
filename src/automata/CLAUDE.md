...
| `tdfa.rs` | Tagged DFA: priority-ordered subset construction tracking a tag→`InputMark` map per NFA thread. Anchored + unanchored start states; `AnchorConditional` (`$`), `anchor_alts` (multiline `^`); `compile_moves`→`MoveOp` fast path. | `Tdfa::try_from` :1081 (correct, **unoptimized**), `Tdfa::optimize` :1234 (opt-in). `Error::PredicatedEpsNotSupported` :40 |
