...
| `tdfa_backend.rs` | Anchored TDFA byte-loop executor. Reusable mark buffers via `MarkScratch` (a `Narrow(Scratch<u32>)`/`Wide(Scratch<usize>)` enum whose width is chosen once from the haystack length, so `find_iter` is allocation-free at either width); leftmost accept with Laurikari-style fallback snapshot. | `execute`, `run_anchored_dyn`, `MarkScratch`, `Scratch` |
