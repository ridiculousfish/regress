...
| `tdfa_backend.rs` | Anchored TDFA byte-loop executor, generic over `TdfaTables` (a borrowed-view trait over every table/flag the loop reads) so the same monomorphized loop drives both a heap-built `Tdfa` and the AoT table tier's `static` tables (`codegen_rt::StaticTdfa`) — one implementation, no drift. Marks are `usize` byte offsets (single-impl `MarkElem`); one reusable `Scratch<usize>` per executor keeps `find_iter` allocation-free per match. Leftmost accept with Laurikari-style fallback snapshot. | `execute`, `run_anchored_dyn`, `TdfaTables`, `Scratch` |
...
| `tdfa/rustgen/` | **AoT Rust-source emitter** (feature `codegen`): **two emission tiers**, chosen by `CODEGEN_UNROLL_MAX_STATES` (256) in `emit_expansion`. *Unrolled* (`verify.rs`, small/typical automata): states→`match` arms, peels→`while` loops LLVM vectorizes, `MoveOp`s→local-variable assignments — fast at runtime but rustc-frontend cost is ~linear in code tokens, so it's capped at `CODEGEN_MAX_STATES`/`CODEGEN_MAX_MARKS` (4096). *Table* (`table.rs`, past the threshold): the automaton as `static` data (`__codegen::StaticTdfa`) driven by the shared interpreter loop (`tdfa_backend::execute_reuse_warm` via the `TdfaTables` view trait) — same match behavior as the interpreter by construction, ceiling is just `TDFA_STATE_BUDGET` (65536) since compiling data, not code, is cheap regardless of size. Both tiers serialize the chosen prefilter `Strategy` into a `__codegen::PrefilterSpec` static (needles re-derived from the IR via the `prefilter.rs` detection helpers). Deterministic output; golden snapshots in `rustgen/snapshots/` double as executable oracle tests via `include!` (see `extern crate self as regress` in lib.rs). | `compile_to_rust`, `emit_expansion`, `verify.rs`, `table.rs`, `prefilter.rs`, `tests.rs` |
