...
fn bench_regex_find_all(c: &mut Criterion) {
    ...
    for (text_name, text) in &texts {
        ...
        for (pattern_name, pattern) in PATTERNS {
            ...
            group.bench_with_input(
                BenchmarkId::new("find_all", bench_name),
                text,
                |b, text| {
                    b.iter(|| {
...
fn bench_pathological_cases(c: &mut Criterion) {
    ...
    let pathological_patterns = [
        ("nested_quantifiers", r"(a+)+b", "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"),
        ("alternation_backtrack", r"(a|a)*b", "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"),
        ("deep_recursion", r"((a)*)*b", "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"),
    ];
    ...
    for (name, pattern, input) in &pathological_patterns {
        ...
        group.bench_with_input(
            BenchmarkId::new("pathological", name),
            input,
            |b, input| {
                b.iter(|| {
...
fn bench_case_insensitive(c: &mut Criterion) {
    ...
    for (text_name, text) in &texts {
        ...
        for (pattern_name, pattern) in &ci_patterns {
            let flags = Flags { icase: true, ..Default::default() };
